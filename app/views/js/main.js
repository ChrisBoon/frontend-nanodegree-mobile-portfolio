...
var resizePizzas = function(size) {
  ...
  // Changes the value for the size of the pizza above the slider
    // var sliderLabel used just to reduce repetition of code within case statements for readibility - doesn't affect performance
  ...
  // Returns the size required for the pizza. Called in resizePizzas and set to the variable 'setPizzaPercentage'.
  ...
  var setPizzaPercentage = determineDx(size);

  // Iterates through pizza elements on the page and changes their widths
  function changePizzaSizes(size) {
    var thePizzas = document.querySelectorAll(".randomPizzaContainer"), i;
    for (i = 0; i < thePizzas.length; ++i) {
      thePizzas[i].style.width = setPizzaPercentage;
    }
  }
...
function updatePositions() {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    var moveMe = items[i].basicLeft + 100 * phase ;
    items[i].style.transform = 'translate3D('+ moveMe + 'px ,0,0)';
  }
